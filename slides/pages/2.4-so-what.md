...
`renderComponent` causing this error happen happens because of an optimization -- it shares the same
renderer of the host ember application, if it exists own the passed in owner.
...
This is a bit different from javascript try/catch.

This doesn't exist, but has been explored once by Yehuda. 

This is something that may need RFC to discuss exact behaviors.

In any case, I need it, and hopefully will have time to help ship some version of this.
...
But here is the patch I'm using in the REPL.

I've removed the code around asserting in the event dispatcehr.

which reminds me, encountering this sent me on a mission to eliminate the event dispatcher.

`@wagenet` has been helping a ton with that as well -- it ultimately ends up requiring deprecating
...
[click] [don't read] demo by clicking on the live screen (remember to go through the Debugger to oppen the app.js
(application.gts)
...
them willy-nilly -- you need a time a way to dispose of them -- and a time to dispose of them. 
